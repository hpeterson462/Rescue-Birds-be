...
app.get('/birds', async (req, res) => {
  const data = await client.query(`SELECT b.id, name, number_of_eggs, flies, color, r.rescue_name 
  FROM birds AS b
  JOIN rescues AS r
  ON b.rescue_id = r.id`);
...
app.get('/birds/:id', async (req, res) => {
  ...
  const data = await client.query(`SELECT birds.id, name, number_of_eggs, flies, color, rescue.id AS rescue_name 
  FROM birds
  JOIN rescues AS rescue 
  ON birds.rescue_id=rescue.id 
  WHERE birds.id=$1`, [birdId]);
...
app.put('/birds/:id', async (req, res) => {
  ...
  try {
    ...
    const data = await client.query(`
      UPDATE birds 
      SET name=$1, number_of_eggs=$2, flies=$3, color=$4, rescue_id=$5 
      WHERE birds.id = $6
